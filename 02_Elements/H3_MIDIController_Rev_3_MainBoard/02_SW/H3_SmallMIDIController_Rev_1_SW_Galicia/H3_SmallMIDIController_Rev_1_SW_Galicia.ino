...
void loop() {
  ...
  SWA0 = readSW_Single(SWA0, A0_ON, GEN_OFF, FADESTEPS_C, FADEDELAY_C);
  SWA1 = readSW_Single(SWA1, A1_ON, GEN_OFF, FADESTEPS_C, FADEDELAY_C);
  SWA2 = readSW_Single(SWA2, A2_ON, GEN_OFF, FADESTEPS_C, FADEDELAY_C);
  SWA3 = readSW_Single(SWA3, A3_ON, GEN_OFF, FADESTEPS_C, FADEDELAY_C);

  if(R < 100){R++;}else{R=0;}
  if(G < 100){G++;}else{G=0;}
  if(B < 100){B++;}else{B=0;}
  ledPadStripe.setPixelColor(0, G, R, B);
...
SWPAD readSW_Single(SWPAD SW_Pad, LEDCOL COL_ON, LEDCOL COL_OFF, int FS, int FD){ // Should Remove arraySize somehow
  ...
  if (analogRead(SW_Pad.muxID) > 512){    // If SW Pad activated // TODO Change to digitalRead()
...
void SW_LED_FadeSingle_FULL(SWPAD SW_Pad, LEDCOL COL_ON, LEDCOL COL_OFF, int FS, int FD){
    ...
    for(int h = 0; h <= (FS); h++){ // Fade to ON
      for(int y = 0; y < SW_Pad.ledQ; y++){
        ledPadStripe.setPixelColor(SW_Pad.ledPos + y, (COL_OFF.green - ((GDiff/FS)*(h))), (COL_OFF.red - ((RDiff/FS)*(h))), (COL_OFF.blue - ((BDiff/FS)*(h))));
      }
      ledPadStripe.show(); // Update LEDs
      delay(FD);
    }
    ...
    for(int h = 0; h <= (FS); h++){ // Fade to OFF
      for(int y = 0; y < SW_Pad.ledQ; y++){
        ledPadStripe.setPixelColor(SW_Pad.ledPos + y, (COL_OFF.green - ((GDiff/FS)*(FS-h))), (COL_OFF.red - ((RDiff/FS)*(FS-h))), (COL_OFF.blue - ((BDiff/FS)*(FS-h))));
      }
      ledPadStripe.show(); // Update LEDs
      delay(FD);
    }
...
void SW_LED_CleanCanvas(LEDCOL COL_OFF){
  for (int h = 0; h < 5; h++){
    ...
    ledPadStripe.setPixelColor(SW_ROW_D[h].ledPos, COL_OFF.green, COL_OFF.red, COL_OFF.blue); // Heavy Stuf^*/
  }  
  ledPadStripe.show(); // Update LEDs
}
